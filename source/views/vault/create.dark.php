 ...
 */
?>
<extends:vault:layout title="<?= !empty($isCopy) ? '[[Copy page]]' : '[[Create page]]' ?>" class="wide-content"/>
...
<define:actions>
    ...
    <?php } else {
        if ($source instanceof \Spiral\Pages\Database\Revision) {
            ?>
            <vault:uri target="pages:viewRevision" options="<?= ['id' => $source->primaryKey()] ?>" class="btn-flat  waves-effect"
                       post-icon="trending_flat">
                [[Back]]
            </vault:uri>
        <?php } else { ?>
            <vault:uri target="pages:edit" options="<?= ['id' => $source->primaryKey()] ?>" class="btn-flat  waves-effect" post-icon="trending_flat">
                [[Back]]
            </vault:uri>
        <?php }
    } ?>
            ...
            <vault:form action="<?= vault()->uri('pages:create') ?>">
                ...
                <vault:allowed permission="vault.pages.add">
                    <div class="right-align">
                        <input type="submit" value="<?= !empty($isCopy) ? '[[CREATE COPY]]' : '[[CREATE]]' ?>" class="btn  waves-effect waves-light"/>
                    </div>
                </vault:allowed>
            </vault:form>
        </div>
